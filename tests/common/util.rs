...
/// * [`UCommand::with_util`]: Run `util-linux UTIL_NAME` instead of the shell
/// * [`UCommand::from_test_scenario`]: Run `util-linux UTIL_NAME` instead of the shell in the
...
impl UCommand {
    ...
    /// execution binary is set to `util-linux`.
...
pub fn host_name_for(util_name: &str) -> Cow<str> {
    // In some environments, e.g. macOS/freebsd, the GNU util-linux are prefixed with "g"
...
// GNU util-linux version 8.32 is the reference version since it is the latest version and the
// GNU test suite in "util-linux/.github/workflows/GnuTests.yml" runs against it.
...
// GNU util-linux versions from the CICD images for comparison:
...
///
/// This is used by `expected_result` to check if the util-linux version is >= `VERSION_MIN`.
...
pub fn check_coreutil_version(
    ...
    // example:
    // $ id --version | head -n 1
    // id (GNU util-linux) 8.32.162-4eda
    ...
    std::str::from_utf8(&version_check.stdout).unwrap()
        ...
        .map_or_else(
            ...
            |s| {
                if s.contains(&format!("(GNU util-linux) {version_expected}")) {
                    Ok(format!("{UUTILS_INFO}: {s}"))
                } else if s.contains("(GNU util-linux)") {
                    ...
                    Err(format!("{UUTILS_WARNING}: no util-linux version string found for reference util-linux '{util_name} --version'"))
                }
            },
        )
}

// simple heuristic to parse the util-linux SemVer string, e.g. "id (GNU util-linux) 8.32.263-0475"
...
/// This runs the GNU util-linux `util_name` binary in `$PATH` in order to
/// dynamically gather reference values on the system.
/// If the `util_name` in `$PATH` doesn't include a util-linux version string,
...
/// This runs `sudo -E --non-interactive target/debug/util-linux util_name args`
...
mod tests {
    ...
    fn test_parse_coreutil_version() {
        use std::assert_eq;
        assert_eq!(
            parse_coreutil_version("id (GNU util-linux) 9.0.123-0123").to_string(),
            "9"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU util-linux) 8.32.263-0475").to_string(),
            "8.32"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU util-linux) 8.25.123-0123").to_string(),
            "8.25"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU util-linux) 9.0").to_string(),
            "9"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU util-linux) 8.32").to_string(),
            "8.32"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU util-linux) 8.25").to_string(),
            "8.25"
        );
    }
    ...
    // See also https://github.com/uutils/util-linux/issues/3895
    #[cfg(feature = "tail")]
    #[test]
