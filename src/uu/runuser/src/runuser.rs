...
fn run(
	...
	if is_login {
        if preserve_env {
        	println!(
        		"--preserve-environment is ignored in case `--login` is enabled"
        	);
        }
        cmd.env_clear();
	}
    ...
    for item in whitelist_env {
    	if !["HOME", "USER", "LOGNAME", "SHELL", "PATH"]
	...
	let status = cmd
		...
		.map_err(|e|
			USimpleError::new(
        		126, format!("Failed to wait for process: {}", e)
        	)
		)?;
...
#[cfg(target_os = "linux")]
fn sep_session() -> UResult<()> {
    setsid()
        .map_err(|e|
        	USimpleError::new(
            	1, format!("Failed to set Sid: {}", e)
            )
        )?;
...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    let supp_gids = if let Some(supp_groups)
    	= matches.get_many::<String>("supp_group") {
                ...
                Gid::from_raw(
                    get_group_info(&supp_group)
                        .map_err(|e|
                        	USimpleError::new(
                        		1,
                            	format!("Failed to get supp group info: {}", e)
                            )
                        )?
                        .ok_or(
                           	USimpleError::new(
	                       		1, "Supp group doesn't exist"
                           )
                        )?
                )
            );
	...
	let rest: Vec<String> = matches.get_many::<String>("rest")
		.unwrap_or_default()
		.map(|s| s.to_string())
		.collect::<Vec<_>>().to_vec();
    let (username, path, command_args)
