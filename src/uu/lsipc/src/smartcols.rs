...
use smartcols_sys::{
    ...
    libscols_line, libscols_table, scols_cell_get_data, scols_cell_get_datasiz, scols_free_iter,
    scols_init_debug, scols_line_get_cell, scols_line_get_userdata, scols_line_set_data,
    scols_line_set_userdata, scols_new_iter, scols_new_table, scols_print_table,
    scols_table_enable_export, scols_table_enable_json, scols_table_enable_noheadings,
    scols_table_enable_raw, scols_table_enable_shellvar, scols_table_get_line,
    scols_table_new_column, scols_table_new_line, scols_table_next_column,
    scols_table_set_column_separator, scols_table_set_name, scols_unref_table,
};
...
impl CellRef {
    pub(crate) fn data(&self) -> &[u8] {
        unsafe {
            ...
            let data_size: usize = scols_cell_get_datasiz(self.0.as_ptr());
            if data_ptr.is_null() || data_size == 0 {
                &[]
            } else {
                std::slice::from_raw_parts(data_ptr.cast(), data_size)
            }
        }
    }
}
